...
const MCOMPositionA: mod.Vector = mod.CreateVector(128.7, 184.04, 137.26);
        ...
        if (!gameOver) {
            ...
            JsPlayer.playerInstances.forEach((player) => {
                ...
                if (player && mod.GetObjId(player) >= 0) {
                    if (!gameOver) UpdateScoreUI(player);
...
function UpdateInteracts(player: mod.Player) {
    if (!roundStarted || buyPhase) return;

    let onAttackingTeam =
        mod.GetObjId(mod.GetTeam(player)) == mod.GetObjId(attackingTeam);
    if (onAttackingTeam) {
        if (
            BombData.carryingPlayer != null &&
            mod.GetObjId(player) == mod.GetObjId(BombData.carryingPlayer)
        ) {
            ...
            let playerPos = mod.GetSoldierState(
                player,
                mod.SoldierStateVector.GetPosition,
            );
            ...
            if (
                mcomADist < disabledInteractDistance ||
                mcomBDist < disabledInteractDistance
            )
                mod.EnableInputRestriction(
                    player,
                    mod.RestrictedInputs.Interact,
...
function UpdateScoreUI(player: mod.Player) {
    ...
    if (jsPlayer) jsPlayer.scoreUI?.refresh();
}
...
export async function OnPlayerLeaveGame(playerId: number) {
    try {
        if (
            (!BombData.carryingPlayer ||
                !mod.IsPlayerValid(BombData.carryingPlayer)) &&
            BombData.isBeingCarried
        ) {
    ...
    if (jsPlayer) {
        ...
        if (!jsPlayer.outOfRound) {
            if (teamID == attackingTeamID) {
                ...
                console.log(
                    "Player is deploying on team ",
                    teamID,
                    ". Teleport them to HQ ",
                    mod.GetObjId(attackersHQ),
                    " at position: ",
                    mod.XComponentOf(hqPos),
                    ", ",
                    mod.YComponentOf(hqPos),
                    ", ",
                    mod.ZComponentOf(hqPos),
                );
...
async function SetupRound() {
    ...
    if (BombData.isBeingCarried) {
        if (BombData.carryingPlayer != null)
            BombData.ItemDropped(BombData.carryingPlayer);
        else BombData.ItemDropped();
    }

    let teamsSwitched: boolean =
        roundNum == maxRounds / 2 + 1 ||
        (debugSwitchTeamsEachRound && roundNum != 1);
    ...
    JsPlayer.playerInstances.forEach((player) => {
        ...
        if (jsPlayer) {
            ...
            if (teamsSwitched) jsPlayer.cash = initialCash;
    ...
    if (attackers.length == 0) {
        ...
        mod.SetWorldIconPosition(
            BombData.bombPositionWorldIcon,
            BombData.currentWorldIconPos,
        );
    } else {
        ...
        attackers.forEach((attacker) => {
            if (
                ...
                mod.GetSoldierState(attacker, mod.SoldierStateBool.IsAlive)
            )
        ...
        if (playerAttackers.length == 0) arr = attackers;
        ...
        console.log(
            ...
            arr[rand],
            " on team ",
            mod.GetObjId(mod.GetTeam(arr[rand])),
        );
    }
    ...
    JsPlayer.playerInstances.forEach((player) => {
        // Close the shops
        let jsPlayer = JsPlayer.get(player);
        if (jsPlayer) jsPlayer.store?.close();
    ...
    mod.MoveObjectOverTime(
        tWall1,
        mod.CreateVector(0, wallMoveDelta, 0),
    ...
    if (AIBackfill) {
        ...
        for (let i = 0; i < 5 - attackingTeamSize; i++) {
            console.log(
                "Spawn AI from spawner ID: ",
                mod.GetObjId(mod.GetSpawner(i + 10)),
                " to Team ID  ",
                attackingTeamID,
            );
        ...
        for (let i = 0; i < 5 - defendingTeamSize; i++) {
            console.log(
                "Spawn AI from spawner ID: ",
                mod.GetObjId(mod.GetSpawner(i + 15)),
                " to Team ID ",
                defendingTeamID,
            );
            mod.SpawnAIFromAISpawner(
...
async function EndRound(winners: mod.Team): Promise<void> {
    ...
    console.log(
        "Attacking Team ID is currently: ",
        mod.GetObjId(attackingTeam),
    );
    console.log(
        "Defending Team ID is currently: ",
        mod.GetObjId(defendingTeam),
    );
    roundEnded = true;
    ...
    mod.MoveObject(
        tWall1,
        mod.Subtract(SpawnWallPos1, mod.GetObjectPosition(tWall1)),
        mod.CreateVector(0, 0, 0),
    );
    mod.MoveObject(
        tWall2,
        mod.Subtract(SpawnWallPos2, mod.GetObjectPosition(tWall2)),
        mod.CreateVector(0, 0, 0),
    );
    mod.MoveObject(
        tWall3,
        mod.Subtract(SpawnWallPos3, mod.GetObjectPosition(tWall3)),
        mod.CreateVector(0, 0, 0),
    );
    mod.MoveObject(
        tWall4,
        mod.Subtract(SpawnWallPos4, mod.GetObjectPosition(tWall4)),
        mod.CreateVector(0, 0, 0),
    );
...
export function OnPlayerDied(
    ...
    if (
        BombData.carryingPlayer &&
        mod.IsPlayerValid(BombData.carryingPlayer) &&
        mod.GetObjId(eventPlayer) == mod.GetObjId(BombData.carryingPlayer)
    ) {
        BombData.ItemDropped(BombData.carryingPlayer);
        console.log(
            "ObjectiveItemData.currentWorldIconPos: ",
            mod.XComponentOf(BombData.currentWorldIconPos),
            mod.YComponentOf(BombData.currentWorldIconPos),
            mod.ZComponentOf(BombData.currentWorldIconPos),
        );
    }
    ...
    if (
        BombData.defusingPlayer != null &&
        mod.GetObjId(BombData.defusingPlayer) == mod.GetObjId(eventPlayer)
    ) {
    ...
    if (combatStarted && !roundEnded && !buyPhase) {
        ...
        let survivingTeamMembers: mod.Player[] = GetLivingPlayersOnTeam(
        ...
        console.log(
            ...
            deadPlayerTeamID,
            " Died. They have ",
            survivingTeamMembers.length,
            " remaining teammates.",
        );
        if (survivingTeamMembers.length == 0) {
            if (
                mod.GetSoldierState(eventPlayer, mod.SoldierStateBool.IsManDown)
            ) {
                console.log(
                    "Player that died is still in man down state. FINISH THEM!",
                );
            ...
            if (
                !BombData.isPlanted &&
                mod.GetObjId(attackingTeam) == deadPlayerTeamID
            ) {
                console.log(
                    "All attackers are dead and the bomb is not planted.",
                );
                EndRound(defendingTeam);
            } else if (
                BombData.isPlanted &&
                ...
                console.log(
                    "Player on attacking team is killed. Bomb is currently planted, so must be defused before round can end.",
                );
            }
        }
    }
...
function GetLivingPlayersOnTeam(team: mod.Team): mod.Player[] {
    ...
    JsPlayer.playerInstances.forEach((player) => {
        let jsPlayer = JsPlayer.get(player);
        if (
            mod.GetObjId(mod.GetTeam(player)) == teamID &&
            !jsPlayer?.outOfRound
        ) {
            teamArr.push(player);
        }
...
export function OnPlayerUndeploy(eventPlayer: mod.Player): void {
    let jsPlayer = JsPlayer.get(eventPlayer);
    if (jsPlayer) jsPlayer.isDeployed = false;
}
...
export async function OnPlayerInteract(player: mod.Player, interactPoint: any) {
    ...
    if (!jsPlayer) {
        console.log(
            "Interaction attempted with invalid JSPlayer! Interact Point ID is: ",
            id,
        );
        return;
    }
    ...
    if (
        BombData.isPlanted &&
        mod.GetObjId(mod.GetTeam(player)) == mod.GetObjId(defendingTeam)
    ) {
...
class JsPlayer {
    ...
    constructor(player: mod.Player) {
        ...
        if (debugJSPlayer) {
            console.log(
                "Bomb Defusal Adding Player [",
                mod.GetObjId(this.player),
                "] Creating JS Player: ",
                JsPlayer.playerInstances.length,
            );
        }
    ...
    static removeInvalidJSPlayers(invalidPlayerId: number) {
        if (!gameOver) {
            if (debugJSPlayer) {
                console.log(
                    "Removing Invalid JSPlayers currently: ",
                    JsPlayer.playerInstances.length,
                );
            }
    ...
    updateWalletUI() {
        ...
        mod.SetUITextLabel(
            ...
            MakeMessage(mod.stringkeys.playerwallet, this.cash),
        );
    }
...
class ScoreUI {
    ...
    refresh() {
        ...
        if (roundStarted) {
            ...
            if (
                mod.GetObjId(attackingTeam) ==
                mod.GetObjId(mod.GetTeam(this.#jsPlayer.player))
            ) {
                mod.SetUITextLabel(
                    this.#teamDesignation,
                    MakeMessage(mod.stringkeys.attackingTeam),
                );
            } else {
                mod.SetUITextLabel(
                    this.#teamDesignation,
                    MakeMessage(mod.stringkeys.defendingTeam),
                );
            }
        } else {
            ...
            mod.SetUITextLabel(
                this.#teamDesignation,
                MakeMessage(mod.stringkeys.buyPhase),
            );
        ...
        mod.SetUITextLabel(
            this.#roundTime,
            MakeMessage(mod.stringkeys.roundTime, minutes, seconds1, seconds2),
        );
        ...
        if (mod.GetObjId(mod.GetTeam(this.#jsPlayer.player)) == 1) {
            mod.SetUITextLabel(
                this.#allyTeamScore,
                MakeMessage(mod.stringkeys.score, team1Score),
            );
            mod.SetUITextLabel(
                this.#enemyTeamScore,
                MakeMessage(mod.stringkeys.score, team2Score),
            );
        } else {
            mod.SetUITextLabel(
                this.#allyTeamScore,
        ...
        mod.SetUITextColor(this.#allyTeamScore, mod.CreateVector(0, 0.2, 1));
            ...
            children: [
                {
                    ...
                    position: [0, 0, 0],
                    size: [
                        this.#containerWidth - this.#backgroundSpacing * 2,
...
class ProgressBar {
    ...
    open(label: mod.Message) {
        if (!this.#rootWidget) this.#create(label);
        if (!this.#rootWidget) return;
    ...
    refresh(progress: number) {
        ...
        if (this.#fillWidget)
            mod.SetUIWidgetSize(
                this.#fillWidget,
                mod.CreateVector(
                    progress * this.#containerWidth,
                    this.#containerHeight,
                    1,
                ),
            );
        else console.log("fill widget is null!");
    }
            ...
            children: [
                {
                    ...
                    position: [0, 0],
                    size: [
                        this.#containerWidth - this.#backgroundSpacing,
                        this.#containerHeight - this.#backgroundSpacing,
                    ],
                    anchor: mod.UIAnchor.Center,
                    bgFill: mod.UIBgFill.Blur,
                    bgColor: BLACKCOLOR,
    ...
    refresh() {
        ...
        if (combatCountdownStarted) {
            mod.SetUITextLabel(
                ...
                MakeMessage(
                    mod.stringkeys.combatStartDelayCountdown,
                    combatStartDelayRemaining,
                ),
            );
        } else {
...
class MessageUI {
    ...
    open(message: mod.Message, textColor: number[]) {
        ...
        if (!this.#rootWidget) this.#create(message, textColor);
        else {
...
class BombData {
    ...
    static Initialize() {
        ...
        mod.SetWorldIconText(
            this.bombPositionWorldIcon,
            MakeMessage(mod.stringkeys.bomb),
        );
    ...
    static ItemProximityCheck() {
        ...
        if (!this.isBeingCarried && this.canBePickedUp) {
            const player = mod.ClosestPlayerTo(
    ...
    static ItemPickedUp(player: mod.Player) {
        ...
        mod.AddUIIcon(
            ...
            2.0,
            mod.CreateVector(0, 1, 0.1),
            mod.Message("Bomb"),
            mod.GetTeam(this.carryingPlayer),
        );
        mod.EnableWorldIconImage(this.bombPositionWorldIcon, false);
        mod.EnableWorldIconText(this.bombPositionWorldIcon, false);
        ...
        JsPlayer.playerInstances.forEach((playerInst) => {
            if (
                ...
                mod.GetObjId(mod.GetTeam(playerInst))
            ) {
                let jsPlayer = JsPlayer.get(playerInst);
                if (jsPlayer) {
                    MessagePlayer(
                        jsPlayer,
                        MakeMessage(mod.stringkeys.bombPickedUp),
                        WHITECOLOR,
                    );
    ...
    static ItemHeld() {
        if (!this.isBeingCarried || this.isPlanted) return;
    ...
    static BombPlanted() {
        if (!this.isPlanted || roundEnded || !roundStarted) return;
    ...
    static ItemDropped(player?: mod.Player) {
        ...
        this.bombModel = mod.SpawnObject(
            mod.RuntimeSpawn_Capstone.RetroBoomBox_01,
            mod.Add(this.lastCarryingPlayerPos, mod.CreateVector(0, 0.25, 0)),
            ZEROVEC,
        );
        ...
        console.log(
            "Bomb World Icon should be at position: ",
            mod.XComponentOf(this.currentWorldIconPos),
            " ",
            mod.YComponentOf(this.currentWorldIconPos),
            " ",
            mod.ZComponentOf(this.currentWorldIconPos),
        );
        BombDropTimer();
    }
    ...
    static PlantBomb() {
        ...
        if (this.carryingPlayer) mod.RemoveUIIcon(this.carryingPlayer);
        ...
        if (this.carryingPlayer != null) {
            ...
            console.log(
                "Bomb was Planted but had no carrying player. This should not be possible.",
            );
        ...
        if (BombData.isAtA) {
            ...
            console.log(
                "Bomb is at neither A nor B when planted! This should not be possible.",
            );
        }
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Bottom_Classic_Grip_Pod,
    assaultRiflePackage_Standard,
);
mod.AddAttachmentToWeaponPackage(
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Magazine_30rnd_Magazine,
    assaultRiflePackage_Basic_B36A4,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Scope_Iron_Sights,
    assaultRiflePackage_Basic_B36A4,
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Barrel_480mm_Factory,
    assaultRiflePackage_Basic_B36A4,
);
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Magazine_30rnd_Magazine,
    assaultRiflePackage_Standard_B36A4,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Scope_1p87_150x,
    assaultRiflePackage_Standard_B36A4,
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Magazine_30rnd_Fast_Mag,
    assaultRiflePackage_Elite_B36A4,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Scope_R4T_200x,
    assaultRiflePackage_Elite_B36A4,
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Magazine_20rnd_Fast_Mag,
    carbinePackage_Elite,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Scope_ST_Prisim_500x,
    carbinePackage_Elite,
);
mod.AddAttachmentToWeaponPackage(
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Bottom_Compact_Handstop,
    PDWPackage_Standard_SMG_PW7A2,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Barrel_180mm_Standard,
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Barrel_180mm_Prototype,
    PDWPackage_Elite_SMG_PW7A2,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Right_5_mW_Red,
...
mod.AddAttachmentToWeaponPackage(
    ...
    DMRPackage_Basic_SVDM,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Scope_Baker_300x,
    DMRPackage_Basic_SVDM,
);
mod.AddAttachmentToWeaponPackage(
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Magazine_10rnd_Fast_Mag,
    DMRPackage_Standard_SVDM,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Scope_LDS_450x,
    DMRPackage_Standard_SVDM,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Bottom_6H64_Vertical,
    DMRPackage_Standard_SVDM,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Barrel_550mm_Factory,
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Magazine_20rnd_Magazine,
    DMRPackage_Elite_SVDM,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Scope_PAS_35_300x,
    DMRPackage_Elite_SVDM,
);
mod.AddAttachmentToWeaponPackage(
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Barrel_620mm_Classic,
    DMRPackage_Elite_SVDM,
);
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Barrel_24_Full,
    SniperPackage_Basic_M2010ESR,
);
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Barrel_650mm_Factory,
    SniperPackage_Standard_SV_98,
);

let SniperPackage_Standard_M2010ESR = mod.CreateNewWeaponPackage();
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Magazine_5rnd_Fast_Mag,
    SniperPackage_Standard_M2010ESR,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Scope_SSDS_600x,
    SniperPackage_Standard_M2010ESR,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Bottom_Bipod,
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Ammo_Match_Grade,
    SniperPackage_Elite_M2010ESR,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Muzzle_Long_Suppressor,
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Barrel_26_Carbon,
    SniperPackage_Elite_M2010ESR,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Left_Range_Finder,
    SniperPackage_Elite_M2010ESR,
);
mod.AddAttachmentToWeaponPackage(
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Magazine_50rnd_Loose_Belt,
    LMGPackage_Basic_M240L,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Scope_Iron_Sights,
    LMGPackage_Basic_M240L,
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Barrel_20_Lima,
    LMGPackage_Basic_M240L,
);
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Magazine_100rnd_Belt_Box,
    LMGPackage_Elite_M240L,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Scope_LDS_450x,
    LMGPackage_Elite_M240L,
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Muzzle_Linear_Comp,
    LMGPackage_Elite_M240L,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Barrel_24_Bravo,
    LMGPackage_Elite_M240L,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Right_120_mW_Blue,
    LMGPackage_Elite_M240L,
);
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Scope_Iron_Sights,
    ShotgunPackage_Basic,
);

let ShotgunPackage_Basic_185KS_K = mod.CreateNewWeaponPackage();
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Magazine_4rnd_Magazine,
    ShotgunPackage_Basic_185KS_K,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Scope_Iron_Sights,
    ShotgunPackage_Basic_185KS_K,
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Barrel_430mm_Factory,
    ShotgunPackage_Basic_185KS_K,
);

let ShotgunPackage_Standard_185KS_K = mod.CreateNewWeaponPackage();
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Magazine_4rnd_Fast_Mag,
    ShotgunPackage_Standard_185KS_K,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Scope_Mini_Flex_100x,
    ShotgunPackage_Standard_185KS_K,
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Bottom_Folding_Stubby,
    ShotgunPackage_Standard_185KS_K,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Barrel_430mm_Factory,
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Magazine_8rnd_Fast_Mag,
    ShotgunPackage_Elite_185KS_K,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Scope_SU_231_150x,
    ShotgunPackage_Elite_185KS_K,
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Ammo_Slugs,
    ShotgunPackage_Elite_185KS_K,
);
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Muzzle_CQB_Suppressor,
...
mod.AddAttachmentToWeaponPackage(
    mod.WeaponAttachments.Magazine_11rnd_Magazine,
    SidearmPackage_Standard_M45A1,
);
...
class Store {
    ...
    onUIButtonEvent(widget: Widget, event: any) {
        ...
        // click on a tab?
        if (
            widgetName.slice(0, this.#tabNamePrefix.length) ==
            this.#tabNamePrefix
        ) {
            //mod.TriggerAudio( mod.SoundEvents2D.ShowObjective, this.#jsPlayer.player);
            let tabIndex = parseInt(
                widgetName.slice(this.#tabNamePrefix.length),
            );
        ...
        if (this.#itemButtons.hasOwnProperty(widgetName)) {
            ...
            this.#jsPlayer.itemBuysPerRound[itemData.id] =
                (this.#jsPlayer.itemBuysPerRound[itemData.id] || 0) + 1;
            this.refresh();
            this.#jsPlayer.player;
    ...
    selectTab(tabIndex: number) {
        if (
            tabIndex < 0 ||
            tabIndex >= this.#tabWidgets.length ||
    ...
    #createTabs() {
        ...
        let widthForTabs =
            this.#storeWidth - this.#tabSpacing * 2 - this.#headerHeight;
    ...
    #createTab(tabIndex: number, tabWidth: number, tabData: any) {
        ...
        let headerPosition = [
            this.#tabSpacing + tabIndex * tabWidth,
            -this.#tabHeight,
        ];
    ...
    #createItems(tabIndex: number, itemDatas: any[], randomize: boolean) {
        ...
        let isItemFiltered = function (jsPlayer: JsPlayer, itemData: any) {
            ...
            try {
                return (
                    itemData.hasOwnProperty("filter_weapon") &&
                    !mod.HasEquipment(jsPlayer.player, itemData.filter_weapon)
                );
            } catch (e) {
        ...
        if (randomize == true) {
            let isItemRandomized = function (itemData: any) {
                return (
                    !itemData.hasOwnProperty("itemRandomized") ||
                    itemData.itemRandomized == true
                );
            };
            ...
            for (
                ...
                if (
                    selectedItems.includes(randomItem) ||
                    !isItemRandomized(itemDatas[randomItem])
                ) {
                    itemIndex--;
                } else {
                    this.#createItem(
    ...
    #createItem(tabIndex: number, itemData: any, itemIndex: number) {
        ...
        let x =
            50 + (1 + xi) * this.#itemSpacingHorizontal + xi * this.#itemSizeH;
...
class ItemButton {
    ...
    constructor(
        ...
        let disabledMsg = itemData.getDisabledMessageCallback?.(
            jsPlayer,
            itemData,
        );
        this.#isDisabled = disabledMsg != null;
        let canAffordIt = this.itemData.canAffordIt(
            this.#jsPlayer,
            this.itemData,
        );
        this.#widget = ParseUI({
            ...
            children: [
                ...
                },
                {
                    ...
                    bgAlpha: 0.8,
                    textLabel:
                        disabledMsg ?? mod.stringkeys.store.disabledMsg.locked,
                    textAnchor: mod.UIAnchor.TopCenter,
                    textSize: 30,
                    textAlpha: 1,
                },
        ...
        if (itemData instanceof StoreItemDataWeapon) {
            ...
            mod.AddUIWeaponImage(
                itemData.name,
                ZEROVEC,
                mod.CreateVector(size[0], size[1], 1),
                mod.UIAnchor.Center,
                itemData.weapon,
                this.#widget,
            );
            ...
            mod.AddUIGadgetImage(
                itemData.name,
                ZEROVEC,
                mod.CreateVector(size[0] * 0.7, size[1] * 0.7, 1),
                mod.UIAnchor.Center,
                itemData.weapon,
                this.#widget,
            );
        ...
        this.#widgetButton = mod.FindUIWidgetWithName(
            itemData.id,
            this.#widget,
        );
        this.#widgetCost = mod.FindUIWidgetWithName(
            this.#widgetCostName,
            this.#widget,
        );
        this.#widgetDisabled = mod.FindUIWidgetWithName(
            this.#widgetDisabledName,
            this.#widget,
        );
        this.refresh();
    }
    ...
    refresh() {
        ...
        mod.SetUITextLabel(
            this.#widgetCost,
            MakeMessage(mod.stringkeys.store.costFmt, this.itemData.cost),
        );
        // disabled?
        let wasDisabled = this.#isDisabled;
        let disabledMsg = this.itemData.getDisabledMessageCallback?.(
            this.#jsPlayer,
            this.itemData,
        );
        ...
        mod.SetUITextColor(
            this.#widgetCost,
            canAffordIt ? mod.CreateVector(1, 1, 1) : mod.CreateVector(1, 0, 0),
        );
    }
}
