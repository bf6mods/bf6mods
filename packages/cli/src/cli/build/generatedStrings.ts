...
export function extractBf6Strings(bf6Strings: Record<string, string>, generateFromLiterals: boolean): Plugin {
	return {
		...
		generateBundle(options, bundle) {
			...
			let id = 0;
			let existingStrings = new Set(Object.values(bf6Strings));
		...
		moduleParsed(moduleInfo) {
			if (moduleInfo.code && moduleInfo.exports.includes('bf6Strings')) {
				...
				const getSnippet = (start: number, end: number, linesBefore = 2, linesAfter = 2) => {
