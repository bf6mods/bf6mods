...
const clients = await new Clients().authenticate({
  authCode: "web-3508238b-7b73-4443-8325-2e6f4a16c511",
});
...
for (const mutator of mutators) {
  ...
  let specificKindData = undefined; // To hold the inner object for range checking
  ...
  if (available?.sparseValues && Array.isArray(available.sparseValues.values)) {
    docLines.push(
